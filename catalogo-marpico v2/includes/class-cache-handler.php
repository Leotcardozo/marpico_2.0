<?php
class Mpc_Cache_Handler {
    private $cache_key = 'mpc_products_cache';
    ...
    public function __construct() {
        ...
        add_action('mpc_api_cleanup_cache', array($this, 'cleanup_cache'));
    }

    // Programar limpieza de caché diaria
    public function schedule_cache_cleanup() {
        if (!wp_next_scheduled('mpc_api_cleanup_cache')) {
            wp_schedule_event(time(), 'daily', 'mpc_api_cleanup_cache');
        }
    }
    ...
    public static function deactivation() {
        wp_clear_scheduled_hook('mpc_api_cleanup_cache');
    }
}

// Registrar función de desactivación
register_deactivation_hook(__FILE__, array('Mpc_Cache_Handler', 'deactivation'));